...
# Note via Mackenly: This is a test file that was used in development.
...
def propogationcalculator(ratio, zenith, spacing, orientation, height):
    """propogationcalculator takes in the ratio, zenith, spacing, orientation, and height as parameters and uses them to
       initialise other variables that are used to perform calculations in the main method

       Args:
           ratio(float): ratio is a parameter of type float that was used to initialise the variable fcc_fi
           zenith(integer): zenith is a parameter of type integer used to initialise the variable theta
           spacing(float): spacing is a parameter of type float used to initialise the variable fcc_si
           orientation(float): orientation is a parameter of type float used to initialise the variable fcc_phi
           height(float): height is a paramater of type float that used to initialise the variable fcc_electricalheight

       Returns:
           result of a calculation that is of the type float
    """
...
def main():
    """The main method is the point of execution for this program
       which includes some if/else statements for determining the top load status,
       and an if/else statement to determine if the system is auto then finally prints out the pointlist array using a
       for loop
    """
    if input("Is the system top loaded? (y/n) ") == "y":
        ...
        if input("Auto input? (y/n): ") == "n":
            ...
            for i in range(361):
                # create tuple called point to hold the calculated data results
                point = i, propogationcalculator(1, .1, .1, .1, 120)
